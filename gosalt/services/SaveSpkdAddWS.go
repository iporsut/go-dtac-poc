...
func DecodeResponseBody(body io.Reader) (*SaveSpkdAddWSResponse, *FaultResponse, error) {
	...
	//body, errr := decoder.CharsetReader("UTF-8", body)

	//if errr != nil {
	//	println("charset decode error!")
	//	return nil, nil, errr
	//}
	...
	for {
		...
		case xml.StartElement:
			...
			if startElement.Name.Space == "http://schemas.xmlsoap.org/soap/envelope/" && startElement.Name.Local == "Fault" {

				fmt.Printf("\nTux Body: %#v\n", startElement)
				enc := xml.NewEncoder(os.Stdout)
				enc.Indent("  ", "    ")
				if err := enc.Encode(startElement); err != nil {
					fmt.Printf("startElement: %v\n", err)
				}
			}
		}
